...
void alsaplay_refresh_outputs(void) {
  ...
  if (!seq_handle) {
    lprintf(LO_WARN,
            "alsaplay_refresh_outputs: Can't list ALSA output ports: "
            "seq_handle is not initialized\n");
    return;
  }
  ...
  while (snd_seq_query_next_client(seq_handle, cinfo) == 0) {
    ...
    while (snd_seq_query_next_port(seq_handle, pinfo) == 0) {
      ...
      // client name only up to 100 chars, so it always fits within a 120 byte
      // buffer
      sprintf(alsaplayer_outputs[out_ind].name, "%.*s (%d:%d)", 100,
              client_name, client_num, port_num);
    }
  }
...
int alsaplay_connect_output(int which) {
  if (which >= alsaplayer_num_outs) {
    lprintf(LO_WARN,
            "alsaplay_connect_output: tried to connect to output listing at "
            "index out of bounds: %d\n",
            which);
    return -1;
  }

  return alsa_midi_set_dest(alsaplayer_outputs[which].client,
                            alsaplayer_outputs[which].port);
}
...
int alsa_midi_default_dest(void) {
  ...
  static const char *loopback_check_name =
      "MIDI THROUGH";  // uppercase for comparison
  ...
  if (!seq_handle) {
    lprintf(LO_WARN,
            "alsa_midi_default_dest: Can't list ALSA output ports: seq_handle "
            "is not initialized\n");
    return 0;
  }
    ...
    while (snd_seq_query_next_port(seq_handle, pinfo) == 0) {
      ...
      if (loopback_cl == client_num) {
        ...
        if (alsa_midi_set_dest(client_num, port_num) != 0) {
          lprintf(LO_WARN,
                  "alsa_midi_default_dest: error connecting to default port "
                  "%i:%i (%s): %s\n",
                  client_num, port_num, client_name,
                  snd_strerror(alsaplayer_err));
        ...
        lprintf(
            LO_INFO,
            "alsa_midi_default_dest: connected to default port %i:%i (%s)\n",
            client_num, port_num, client_name);
  ...
  if (loopback_cl != -1) {
    if ((status = alsa_midi_set_dest(loopback_cl, loopback_prt)) != 0) {
      lprintf(LO_WARN,
              "alsa_midi_default_dest: (fallback) error connecting to default "
              "port %i:%i (%s): %s\n",
    ...
    lprintf(LO_INFO,
            "alsa_midi_default_dest: (fallback) connected to default port "
            "%i:%i (%s)\n",
...
int alsa_midi_set_dest(int client, int port) {
  ...
  CHK_LPRINT_ERR_RET(snd_seq_connect_to(seq_handle, out_port, client, port), -3,
                     LO_WARN,
                     "alsa_midi_set_dest: error connecting to (%d:%d): %s",
                     last_client, last_port);
...
static unsigned long alsa_now(void) {
  ...
  // update queue status
  CHK_LPRINT_ERR_RET(
      snd_seq_get_queue_status(seq_handle, out_queue, queue_status), 0, LO_WARN,
  ...
  if (time == 0) {
    lprintf(LO_WARN,
            "alsaplayer: alsa_now(): error getting realtime position from "
            "queue status\n");
    return 0;
  }
...
static void alsa_midi_evt_finish() {
  CHK_LPRINT_ERR(
      snd_seq_event_output(seq_handle, &seq_ev), LO_WARN,
      "alsa_midi_evt_finish: could not output alsa midi event: %s\n");
}

static void alsa_midi_evt_flush() {
  CHK_LPRINT_ERR(
      snd_seq_drain_output(seq_handle), LO_WARN,
      "alsa_midi_evt_finish: could not drain alsa sequencer output: %s\n");
}

static void alsa_midi_write_event(unsigned long when, midi_event_type_t type,
...
static void alsa_midi_write_control(unsigned long when, int channel, int v1,
                                    int v2) {
...
static int alsa_midi_init_connect_default_port(void) {
  ...
  if (snd_mididev && strlen(snd_mididev)) {
    ...
    CHK_LPRINT_ERR(
        snd_seq_parse_address(seq_handle, &seqaddr, snd_mididev), LO_WARN,
...
static void alsa_play(const void *handle, int looping) {
  ...
  if (status < 0) {
    lprintf(
        LO_WARN,
        "alsa_play: error setting sched queue timer with new resolution: %s\n",
        snd_strerror(status));
...
static void alsa_midi_writesysex(unsigned long when, int etype,
...
static void alsa_render(void *vdest, unsigned bufflen) {
  ...
  while (1) {
    ...
    switch (currevent->event_type) {
      ...
      case MIDI_EVENT_SYSEX_SPLIT:
        alsa_midi_writesysex(when, currevent->event_type,
      ...
      case MIDI_EVENT_CONTROLLER:
        if (currevent->data.channel.param1 == 7) {  // volume event
          alsa_setchvolume(currevent->data.channel.channel,
                           currevent->data.channel.param2, when);
          break;
        }  // fall through
      default:
        alsa_midi_write_event(
  ...
  if (samples + sampleswritten >
      length) {  // broke due to next event being past the end of current render
                 // buffer
...
const music_player_t alsa_player = {
    alsa_name,  alsa_init,   alsa_shutdown,     alsa_setvolume,
