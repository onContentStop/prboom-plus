...
static void SetDefaultSaveName(int slot) {
  ...
  if (lumpinfo[maplumpnum].source == source_iwad) {
    ...
    snprintf(savegamestrings[itemOn], SAVESTRINGSIZE, "%s (%s)",
             lumpinfo[maplumpnum].name, basename);
    (free)(wadname);
  }
...
void M_DrawMouse(void) {
  ...
  // e6y
  {
    ...
    M_DrawThermo(MouseDef.x, MouseDef.y + LINEHEIGHT * (mouse_mlook + 1), 100,
                 mpmx);
    mpmx = mouse_acceleration > 99 ? 99 : mouse_acceleration;
    M_DrawThermo(MouseDef.x, MouseDef.y + LINEHEIGHT * (mouse_accel + 1), 100,
                 mpmx);
  }
}
...
setup_menu_t stat_settings2[] = {
    ...
    {"SHOW TIME/STS ABOVE STATUS BAR",
     S_YESNO,
     m_null,
     ADVHUD_X,
     SB_Y + 8 * 8,
...
// e6y
    ...
    if (V_GetMode() == VID_MODEGL) {
      if (ch == key_showalive) {
        ...
        doom_printf(
            ...
            (show_alive ? (show_alive == 1 ? "(mode 1) on" : "(mode 2) on")
                        : "off"));
      }
    }
#endif
    ...
    if (ch == key_novert) {
      ...
      doom_printf("Vertical Mouse Movement %s",
                  movement_mousenovert ? "off" : "on");
      // Don't eat the keypress in this case.
      // return true;
    }
