...
void G_BuildTiccmd(ticcmd_t *cmd)
{
    ...
    speed =
        (gamekeydown[key_speed] || joybuttons[joybspeed] ? !autorun
                                                         : autorun); // phares
        ...
        else
        {
            newweapon =
                (gamekeydown[key_weapon1] || mousebuttons[mb_weapon1])
                    ...
                    (gamekeydown[key_weapon2] || mousebuttons[mb_weapon2])
                    ? wp_pistol
                : (gamekeydown[key_weapon3] || mousebuttons[mb_weapon3])
                    ? wp_shotgun
                : (gamekeydown[key_weapon4] || mousebuttons[mb_weapon4])
                    ? wp_chaingun
                : (gamekeydown[key_weapon5] || mousebuttons[mb_weapon5])
                    ? wp_missile
                ...
                : (gamekeydown[key_weapon7] || mousebuttons[mb_weapon7]) &&
                        gamemode != shareware
                    ? wp_bfg
                : (gamekeydown[key_weapon8] || mousebuttons[mb_weapon8])
                    ? wp_chainsaw
                : (!demo_compatibility &&
    ...
    {
        side +=
            mousex / movement_mousestrafedivisor; /* mead  Don't want to strafe
                                                     as fast as turns.*/
...
dboolean G_Responder(event_t *ev)
{
    ...
    case ev_mouse:
        ...
        if (GetMouseLook())
            if (movement_mouseinvert)
                mlooky +=
                    (AccelerateMouse(ev->data3) * (mouseSensitivity_mlook)) /
                    10;
            else
                mlooky -=
                    (AccelerateMouse(ev->data3) * (mouseSensitivity_mlook)) /
    ...
    else
    {
        ...
        if (demofp)
        {
            ...
            if (pos)
            {
                ...
                if (slot != -1)
                {
                    /* Return to the last save position, and load the relevant
                     * savegame */
            ...
            {
                // restoration of all data which could be changed by
                // G_ReadDemoHeader
...
void P_WalkTicker()
{
    ...
    strafe = gamekeydown[key_strafe] || mousebuttons[mousebstrafe] ||
             joybuttons[joybstrafe];
    speed =
        autorun || gamekeydown[key_speed] || joybuttons[joybspeed]; // phares
