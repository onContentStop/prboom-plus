...
if __name__ == "__main__":
    ...
    try:
        with open("unions.hh", "w") as o:
            with open("unions.cc", "w") as o2:
                o.write("#pragma once\n")
                o.write("#include <ostream>\n")
                o2.write("#include \"unions.hh\"\n")
                for line in i_lines:
                    line = line.strip()
                    line_num += 1
                    if line == "":
                    ...
                    if state == State.start:
                        ...
                        if line[0] == "variants":
                            ...
                            o.write("}")
                            o.write(
                                f"friend std::ostream &operator<<(std::ostream &os, const {union_name} &{union_name.lower()});")
                            o.write("};")
                            o.write(f"constexpr {union_name} {union_name.upper()}_NULL;")
                            o2.write(
                                f"std::ostream &operator<<(std::ostream &os, const {union_name} &{union_name.lower()}){{")
                            o2.write(f"switch ({union_name.lower()}.m_discriminant) {{")
                            for var in variants:
                                o2.write(f"case {union_name}::discriminant::{var.name}:")
                                o2.write(
                                    f"return os << \"{union_name} {{ kind: {var.name}, pointer: \" << {union_name.lower()}.m_{var.name} << \" }}\";")
                            o2.write(f"case {union_name}::discriminant::undefined:")
                            o2.write(f"return os << \"{union_name} {{ kind: undefined }}\";")
                            o2.write("}")
                            o2.write("}")
                            types.clear()
                            variants.clear()
