...
fixed_t P_FindShortestTextureAround(int secnum) {
  ...
  if (!comp[comp_model]) {
    minsize = 32000 << FRACBITS;  // jff 3/13/98 prevent overflow in height
  }
  // calcs
...
void P_ShootSpecialLine(mobj_t *thing, line_t *line) {
  ...
  if (!demo_compatibility) {
    ...
    if ((unsigned)line->special >= GenEnd) {
      ...
      if (!comperr(comperr_zerotag) &&
          !line->tag) {  // e6y //jff 2/27/98 all gun generalized types require
                         // tag
        return;
      }
      ...
      if (!comperr(comperr_zerotag) &&
          !line->tag) {  // e6y //jff 3/2/98 all gun generalized types require
                         // tag
        return;
      }
      linefunc = EV_DoGenDoor;
      ...
      if (((line->special & TriggerType) == GunOnce) ||
          ((line->special & TriggerType) ==
           GunMany)) {  // jff 4/1/98 check for being a gun type before
                        // reporting door type
        if (!P_CanUnlockGenDoor(line, thing->player)) {
          return;
        }
...
void P_PlayerInSpecialSector(player_t *player) {
    ...
    switch (sector->special) {
      ...
      case 9:
        ...
        if (hudadd_secretarea) {
          int sfx_id = (I_GetSfxLumpNum(&S_sfx[sfx_secret]) < 0 ? sfx_itmbk
                                                                : sfx_secret);
  ...
  {
    if (mbf21 && sector->special & INSTANT_KILL_MASK) {
...
static void Add_WallScroller(fixed_t dx, fixed_t dy, const line_t *l,
  ...
  if (compatibility_level >= lxdoom_1_compatibility) {
    x = (fixed_t)(((int_64_t)dy * -(int_64_t)l->dy -
                   (int_64_t)dx * (int_64_t)l->dx) /
                  (int_64_t)d);  // killough 10/98:
    y = (fixed_t)(((int_64_t)dy * (int_64_t)l->dx -
                   (int_64_t)dx * (int_64_t)l->dy) /
