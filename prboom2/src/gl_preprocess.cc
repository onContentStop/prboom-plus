    ...
    for (i = 0; i < numclippers; i++)
    {
        ...
        for (k = 0; k < num; k++)
        {
            ...
            if (sidelist[startIdx] != sidelist[endIdx])
            {
                ...
                points = (vertex_t *)Z_Realloc(
                    points, (++num) * sizeof(vertex_t), PU_LEVEL, 0);
                if (num >= MAX_CC_SIDES)
...
static void CALLBACK ntessBegin(GLenum type)
{
    ...
    sectorloops[currentsector].loops = (GLLoopDef *)Z_Realloc(
...
static void gld_PrecalculateSector(int num)
{
    ...
    currentsector = num;
    lineadded = (dboolean *)Z_Malloc(sectors[num].linecount * sizeof(dboolean),
                                     PU_LEVEL, 0);
    if (!lineadded)
    {
        if (levelinfo)
    ...
    while (linecount)
    {
        ...
        {
            maxvertexnum += 512;
            v = (double *)Z_Realloc(v, maxvertexnum * 3 * sizeof(double),
                                    PU_LEVEL, 0);
        }
...
static void gld_PreprocessSectors(void)
{
    ...
    if (numvertexes)
    {
        vertexcheck = (char *)malloc(numvertexes * sizeof(vertexcheck[0]));
        vertexcheck2 = (char *)malloc(numvertexes * sizeof(vertexcheck2[0]));
