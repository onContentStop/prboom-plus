...
static int P_CheckForZDoomUncompressedNodes(int lumpnum, int gl_lumpnum) {
  ...
  if (result) {
    ...
    if (result) {
      lprintf(LO_INFO,
              "P_CheckForZDoomUncompressedNodes: compressed ZDoom nodes are "
              "detected\n");
...
static void P_LoadSegs(int lump) {
  ...
  for (i = 0; i < numsegs; i++) {
    ...
    if (v1 >= numvertexes || v2 >= numvertexes) {
      char str[200] =
          "P_LoadSegs: compatibility loss - seg %d references a non-existent "
          "vertex %d\n";
...
static void P_LoadSegs_V4(int lump) {
  ...
  for (i = 0; i < numsegs; i++) {
    ...
    if (v1 >= numvertexes || v2 >= numvertexes) {
      char str[200] =
          "P_LoadSegs_V4: compatibility loss - seg %d references a "
...
static void P_LoadZNodes(int lump, int glnodes, int compressed) {
  ...
  if (compressed == ZDOOM_ZNOD_NODES) {
    ...
    if (inflateInit(zstream) != Z_OK)
      I_Error(
          "P_LoadZNodes: Error during ZDoom nodes decompression "
          "initialization!");
...
static void P_LoadBlockMap(int lump) {
  ...
  if (M_CheckParm("-blockmap") || W_LumpLength(lump) < 8 ||
    ...
    if (!P_VerifyBlockMap(count)) {
      ...
      lprintf(LO_INFO,
              "P_LoadBlockMap: use \"-blockmap\" command line switch for "
              "rebuilding\n");
    }
  }
