...
#define F_PANINC                                                               \
    (gamekeydown[key_speed] ? map_scroll_speed * 2 : map_scroll_speed)
...
static void AM_changeWindowLoc(void)
{
    ...
    if (static_cast<bool>(automapmode & am_rotate))
    {
...
{
    if (static_cast<bool>(automapmode & am_overlay))
    {
    ...
    else if (ev->type == ev_keydown)
    {
                 ...
                 (map_wheel_zoom && ch == KEYD_MWHEELDOWN))
        {
            ...
            curr_mtof_zoommul = mtof_zoommul;
                 ...
                 (map_wheel_zoom && ch == KEYD_MWHEELUP))
        {
        ...
        {
            automapmode ^= am_follow; // CPhipps - put all automap mode
                                      // stuff into one enum
            // Ty 03/27/98 - externalized
            plr->message = static_cast<bool>(automapmode & am_follow)
                               ? s_AMSTR_FOLLOWON
                               : s_AMSTR_FOLLOWOFF;
        }
        else if (ch == key_map_grid)
        {
            ...
            plr->message = static_cast<bool>(automapmode & am_grid)
                               ? s_AMSTR_GRIDON
                               : s_AMSTR_GRIDOFF;
        }
        ...
        else if (ch == key_map_rotate)
        {
            ...
            plr->message = static_cast<bool>(automapmode & am_rotate)
                               ? s_AMSTR_ROTATEON
                               : s_AMSTR_ROTATEOFF;
        }
        else if (ch == key_map_overlay)
        {
            ...
            AM_SetPosition();
            AM_activateNewScale();
            plr->message = static_cast<bool>(automapmode & am_overlay)
                               ? s_AMSTR_OVERLAYON
                               : s_AMSTR_OVERLAYOFF;
        }
#ifdef GL_DOOM
    ...
    for (i = 0; i < numlines; i++)
    {
        ...
        if (ddt_cheating || (lines[i].flags & ML_MAPPED))
        {
                ...
                {
                    {
                        ...
                        case 1:
                            /*bluekey*/
                            AM_drawMline(&l, mapcolor_bdor ? mapcolor_bdor
                                                           : mapcolor_cchg);
                            continue;
                        case 2:
                            /*yellowkey*/
                            AM_drawMline(&l, mapcolor_ydor ? mapcolor_ydor
                                                           : mapcolor_cchg);
                            continue;
                        case 0:
                            /*redkey*/
                            AM_drawMline(&l, mapcolor_rdor ? mapcolor_rdor
                                                           : mapcolor_cchg);
                            continue;
                        case 3:
                            /*any or all*/
                            AM_drawMline(&l, mapcolor_clsd ? mapcolor_clsd
                                                           : mapcolor_cchg);
                            continue;
                        }
                    }
            ...
            if (!lines[i].backsector)
            {
                ...
                if (mapcolor_secr && // jff 4/3/98 0 is disable
                    ((map_secret_after && P_WasSecret(lines[i].frontsector) &&
                     ...
                     (!map_secret_after && P_WasSecret(lines[i].frontsector))))
                    AM_drawMline(&l,
                                 mapcolor_secr); // line bounding secret sector
            ...
            else /* now for 2S lines */
            {
                ...
                {
                    AM_drawMline(
                        &l, mapcolor_flat); // 2S lines that appear only in IDDT
                }
            }
    ...
    // marked locations on the automap
    {
             ...
             i++) // killough 2/22/98: remove automap mark limit
        {
            if (markpoints[i].x != -1)
            {
                ...
                gld_AddNiceThing(am_icon_mark, p.fx, p.fy, radius, 0, 255, 255,
                                 0, (unsigned char)anim_flash_level);
            }
        }
    }
...
void AM_Drawer(void)
{
    ...
    if (static_cast<bool>(automapmode & am_follow))
        AM_doFollowPlayer();
    ...
    if (static_cast<bool>(automapmode & am_grid))
