...
void D_DoAdvanceDemo(void) {
  ...
  if (demosequence == 6 && gamemode == commercial &&
      W_CheckNumForName("demo4") < 0)
    demosequence = 0;
...
static char *GetAutoloadDir(const char *base, const char *iwadname,
                            dboolean createdir) {
  char *result;
  int len;
...
static void D_DoomMainSetup(void) {
  ...
  */
  ...
  // process deh files from autoload directory before deh in wads from -file
  // parameter
  ...
  if (slot && ++slot < myargc) {
    ...
    G_LoadGame(
        slot, true);  // killough 5/15/98: add command flag // cph - no filename
