  ...
  if (gld_drawinfo.num_items[GLDIT_TWALL] > 0 ||
    ...
    while (twall_idx >= 0 || tsprite_idx >= 0) {
      ...
      if (twall_idx >= 0 && tsprite_idx >= 0) {
         ...
         * which is farther */
        seg_t *twseg =
            gld_drawinfo.items[GLDIT_TWALL][twall_idx].item.wall->seg;
        int ti;
      ...
      if (draw_tsprite) {
        ...
        glAlphaFunc(GL_GEQUAL, gl_mask_sprite_threshold_f);
        gld_SetFog(gld_drawinfo.items[GLDIT_TSPRITE][tsprite_idx]
                       .item.sprite->fogdensity);
        gld_DrawSprite(
            gld_drawinfo.items[GLDIT_TSPRITE][tsprite_idx].item.sprite);
        tsprite_idx--;
      } else {
        ...
        gld_SetFog(
            gld_drawinfo.items[GLDIT_TWALL][twall_idx].item.wall->fogdensity);
