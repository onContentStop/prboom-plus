...
unsigned char *gld_ReadScreen(void) { // NSM convert to static
  ...
  if (buffer && scr) {
    ...
    glFlush();
    glReadPixels(0, 0, REAL_SCREENWIDTH, REAL_SCREENHEIGHT, GL_RGB,
                 GL_UNSIGNED_BYTE, scr);

    glPixelStorei(GL_PACK_ALIGNMENT, pack_aligment);

    gld_ApplyGammaRamp(scr, REAL_SCREENWIDTH * 3, REAL_SCREENWIDTH,
                       REAL_SCREENHEIGHT);

    for (i = 0; i < REAL_SCREENHEIGHT / 2; i++) {
      memcpy(buffer, &scr[i * REAL_SCREENWIDTH * 3], REAL_SCREENWIDTH * 3);
      memcpy(&scr[i * REAL_SCREENWIDTH * 3],
             &scr[(REAL_SCREENHEIGHT - (i + 1)) * REAL_SCREENWIDTH * 3],
             REAL_SCREENWIDTH * 3);
      memcpy(&scr[(REAL_SCREENHEIGHT - (i + 1)) * REAL_SCREENWIDTH * 3], buffer,
             REAL_SCREENWIDTH * 3);
    }
  }
...
GLvoid gld_Set2DMode(void) {
  ...
  glLoadIdentity();
  glOrtho((GLdouble)0, (GLdouble)REAL_SCREENWIDTH, (GLdouble)REAL_SCREENHEIGHT,
...
void gld_AddWall(seg_t *seg) {
  ...
  if (fake_contrast) {
    rellight = seg->linedef->dx == 0 ? +gl_rellight
                                     : seg->linedef->dy == 0 ? -gl_rellight : 0;
  }
...
void gld_ProjectSprite(mobj_t *thing, int lightlevel) {
  ...
  if (thing->type == MT_BLOOD && thing->target) {
    if (thing->target->type == MT_HEAD) {
      sprite.cm = CR_BLUE2;
    } else if (thing->target->type == MT_SKULL) {
      sprite.cm = CR_GRAY;
