...
static void gld_PreprocessFakeSector(int ceiling, sector_t *sector,
                                     int groupid);
static void gld_RegisterBleedthroughSector(sector_t *source, sector_t *target,
                                           int ceiling);
    ...
    for (i = 0; i < sectors[num].linecount; i++) {
      ...
      if (side0 && side1) {
        ...
        if (side0->sector && side1->sector) {
          ...
          dboolean front_ceil_is_sky =
              (side0->sector->ceilingpic == skyflatnum);
...
static void gld_RegisterBleedthroughSector(sector_t *source, sector_t *target,
  ...
  if ((bleedsectors[source_idx].target == NULL) ||
      (bleedsectors[source_idx].target &&
       ((ceiling && bleedsectors[source_idx].target->ceilingheight >
