...
static void deh_procBexCodePointers(DEHFILE *fpin, FILE *fpout, char *line) {
  ...
  while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
    ...
    {
      ++i;
      if (!stricmp(key, deh_bexptrs[i].lookup)) {  // Ty 06/01/98  - add  to
                                                   // states[].action for new
                                                   // djgcc version
...
static uint_64_t getConvertedDEHBits(uint_64_t bits) {
  static const uint_64_t bitMap[32] = {
      ...
      MF_DROPOFF,  // 10 Travels over cliffs - Monsters normally do not walk off
                   // ledges/steps they could not walk up. With this set they
                   // can walk off any height of cliff. Usually only used for
                   // flying monsters.
...
static void deh_procPars(DEHFILE *fpin, FILE *fpout, char *line)  // extension
{
  ...
  while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
    ...
    if (3 != sscanf(inbuffer, "par %i %i %i", &episode, &level,
                    &partime)) {                                   // not 3
...
static void deh_procText(DEHFILE *fpin, FILE *fpout, char *line) {
  char key[DEH_MAXKEYLEN];
  char
      inbuffer[DEH_BUFFERMAX * 2];  // can't use line -- double size buffer too.
  int i;                            // loop variable
      ...
      tolen < 7)  // lengths of music and sfx are 6 or shorter
  {
    ...
    for (i = 1; i < NUMSFX; i++) {
      ...
      if (!S_sfx[i].names.data[0]) {
        continue;
      }
      // avoid short prefix erroneous match
      if (strlen(S_sfx[i].names.data[0]) != (size_t)fromlen) {
        continue;
      }
