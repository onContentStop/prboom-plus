...
static void cheat_mus(buf) char buf[3];
{
  ...
  doom_printf("%s", s_STSTR_MUS);  // Ty 03/27/98 - externalized
  ...
  if (gamemode == commercial) {
    ...
    if (musnum < mus_runnin || ((buf[0] - '0') * 10 + buf[1] - '0') > 35) {
      doom_printf("%s", s_STSTR_NOMUS);  // Ty 03/27/98 - externalized
    } else {
...
static void cheat_choppers() {
  ...
  doom_printf("%s", s_STSTR_CHOPPERS);  // Ty 03/27/98 - externalized
}
...
static void cheat_god() {  // 'dqd' cheat for toggleable god mode
  ...
  if (plyr->cheats & CF_GODMODE) {
    ...
    doom_printf("%s", s_STSTR_DQDON);  // Ty 03/27/98 - externalized
  } else {
    doom_printf("%s", s_STSTR_DQDOFF);  // Ty 03/27/98 - externalized
  }
}
...
static void cheat_health() {
  if (!(plyr->cheats & CF_GODMODE)) {
    ...
    doom_printf("%s", s_STSTR_BEHOLDX);  // Ty 03/27/98 - externalized
  }
}
...
static void cheat_kfa() {
  cheat_k();
  cheat_fa();
  doom_printf("%s", s_STSTR_KFAADDED);
}
...
static void cheat_noclip() {
  ...
  doom_printf("%s", (plyr->cheats ^= CF_NOCLIP) & CF_NOCLIP
                        ? s_STSTR_NCON
...
static void cheat_behold() {
  doom_printf("%s", s_STSTR_BEHOLD);  // Ty 03/27/98 - externalized
}
...
static void cheat_clev(char buf[3]) {
  ...
  doom_printf("%s", s_STSTR_CLEV);  // Ty 03/27/98 - externalized
...
static void cheat_comp() {
  ...
  doom_printf("New compatibility level:\n%s (%d)",
              comp_lev_str[compatibility_level], compatibility_level);
}

// variable friction cheat
static void cheat_friction() {
  doom_printf("%s",  // Ty 03/27/98 - *not* externalized
              (variable_friction = !variable_friction)
                  ? "Variable Friction enabled"
...
static void cheat_hom() {
  doom_printf("%s", (flashing_hom = !flashing_hom) ? "HOM Detection On"
                                                   : "HOM Detection Off");
}
...
static void cheat_tntkeyxx(int key) {
  doom_printf("%s", (plyr->cards[key] = !plyr->cards[key])
                        ? "Key Added"
...
static void cheat_tntweap() {  // Ty 03/27/98 - *not* externalized
  doom_printf(
      "%s", gamemode == commercial ? "Weapon number 1-9" : "Weapon number 1-8");
}
...
static void cheat_smart() {
  doom_printf("%s", (monsters_remember = !monsters_remember)
                        ? "Smart Monsters Enabled"
                        : "Smart Monsters Disabled");
}

static void cheat_pitch() {
  doom_printf("%s", (pitched_sounds = !pitched_sounds)
...
static void cheat_notarget() {
  ...
  if (plyr->cheats & CF_NOTARGET) {
    doom_printf("Notarget Mode ON");
  } else {
    doom_printf("Notarget Mode OFF");
  }
}
...
static void cheat_fly() {
  if (plyr->mo != NULL) {
    ...
    if (plyr->cheats & CF_FLY) {
      ...
      doom_printf("Fly mode ON");
    } else {
      plyr->mo->flags &= ~MF_NOGRAVITY;
      plyr->mo->flags &= ~MF_FLY;
      doom_printf("Fly mode OFF");
    }
  }
}
